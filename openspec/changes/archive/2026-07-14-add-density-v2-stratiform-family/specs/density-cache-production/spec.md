...
W7 V2 cache update SHALL 继续只 dispatch 现有三维缓存网格；每个有效体素 MUST 保留全局 invocation bounds check 与恰好一次最终 RGBA16F storage write。V2 source MAY 读取 Frame/Body/Recipe Support、只读 tile-body mask 与 group 2 shared fields，并 SHALL 以固定 `i<activeBodyCount<=12` 循环只访问 candidate bit。recipe-enabled、topology family、analytic footprint 与 vertical support early reject MUST 位于 shared sample 前。

W7 source SHALL 只包含 Common Context、一个参数化 Stratiform family evaluator、Cumulus Billow evaluator、静态五属 dispatch、Legacy-compatible soft overlap/top-two metadata 与 final writer。四个 Stratiform genus 每次通过早退的 Body evaluation SHALL 至多执行 Macro=1、Base=1；Cumulus SHALL 保持 Macro=1、Base=2、Detail=1 与一次无额外 sample 的 low warp。其他五属、空 tile 与 rejected Body texture samples SHALL 为零。V2 source MUST NOT 包含 Legacy evaluator/4D Voronoi/fBm、Cellular/Fiber/Convective evaluator、dynamic operator/sample loop、atomics、workgroup storage、occupied-tile compaction、indirect dispatch 或额外正常帧 compute/render pass。

Shared atlas/macro generation SHALL 继续只在首次 candidate warmup或自身 config/seed 失效时编码；普通 W7 cache update、Body movement、wind、mask revision、resolution 或 workgroup 变化 MUST NOT 重建 shared fields。默认 Legacy 且 V2 未请求时，全部 V2 module/pipeline/mask/shared-field/evaluator resources、CPU builders、GPU memory 和 pass count MUST 为零。

#### Scenario: Mask 不跳过最终写入

- **WHEN** W7 tile mask 对某 workgroup tile 为零
- **THEN** invocation SHALL 跳过 body/evaluator/sample 区域，但每个 bounds 内体素仍 MUST 写一次零值，MUST NOT 因 ping-pong 目标复用而保留陈旧密度

#### Scenario: 五属 Compute 静态成本

- **WHEN** 静态审计 W7 V2 cache compute 可达调用图
- **THEN** source SHALL 恰好包含 Stratiform/Cumulus 两个 evaluator family；四个 Stratiform/Cumulus sample-call 上限 SHALL 分别为 2/4，所有循环 SHALL 具有编译期 12-body 上限，其他 family/Legacy/4D/interpreter/atomic source SHALL 为零

#### Scenario: Shared Generator 不进入稳态

- **WHEN** shared-field config/seed 未改变且 V2 candidate 已完成首次 warmup
- **THEN** 普通 cache update SHALL 只编码一个 V2 cache compute pass，不编码 atlas/macro generator；正常 cloud render 与 ground-shadow pass 数 SHALL 与 W6 相同
...
#### Scenario: W7 五属输入生成非零缓存

- **WHEN** V2 输入合法 Cumulus 或任一 Stratiform Body、对应 Recipe enabled 且 voxel 通过 Support/profile/topology
...
- **WHEN** V2 输入一个或多个有效 Body 但其 genus 均属于 Stratocumulus/Cumulonimbus/Altocumulus/Cirrus/Cirrocumulus
...
- **WHEN** active Producer 从 Legacy 切换为 W7 V2
...
### Requirement: W7 Stratiform 诊断与固定 Manifests

Stats/HUD SHALL 报告 W7 enabled genera、unsupported genera、St/Cs/As/Ns/Cu 静态 sample limits、unsupported Body count、tile candidate/voxel-body upper bound、`actualEvaluatorCalls=unavailable`、cache sample ID、timestamp availability、shared generator状态、pipeline/source size 与资源字节。理论 upper bound MUST NOT 被表述为实际调用数或实测加速。

系统 SHALL 提供 St/Cs/As/Ns single、stratiform stack/overlap 的固定 Legacy/V2 cases，保持 camera、scene time、Body、wind、resolution、workgroup、quality 与 render params 一致。Cached normal SHALL 是 cache timing 主样本；Hybrid 与 density-debug SHALL 是视觉/协议证据，不得混入 cache Gate median。

#### Scenario: A/B Producer 对齐

- **WHEN** 运行任一 W7 Legacy/V2 配对 case
...
#### Scenario: Family 资源预算

- **WHEN** 比较 W6 与 W7 steady normal frame
- **THEN** W7 SHALL 不增加 persistent texture/buffer class、bind group index或正常 frame pass，source/Recipe record 增长 SHALL 单独报告
