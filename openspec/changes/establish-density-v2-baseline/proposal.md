...
- 固定十个单云属场景，以及“十属同场景”和“单个复杂 Cb”两个压力场景。
- 固定相机、viewport、场景时间、暂停状态、天气、风、生命周期、云体 placement、质量参数、`96³` cache 与 update rate。
- 分别采集 Cached/Hybrid、正常视图/density debug；Realtime 只做一次代表场景兼容状态记录，不进入性能矩阵。
...
- 生成机器可读 JSON 结果、人类可读索引和按规范命名的截图证据。
...
- W1 及后续 Density V2 change 在 W0 证据包完整前不得开始实施。
