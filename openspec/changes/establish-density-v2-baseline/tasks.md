...
- [ ] 3.1 实现 warm-up 与 sample 状态机；只在 `cacheRan=true` 时采集 cache 样本
- [ ] 3.2 输出各 pass 的 count、median、p95、min、max；normal/debug 与 Cached/Hybrid 独立分组
- [ ] 3.3 `timestamp-query` 不可用时输出 `unavailable`，不得以 FPS/CPU 时间代替 GPU timing
- [ ] 3.4 导出包含 revision、active changes、device、fingerprint、case、stats、warnings 和 screenshot path 的 versioned JSON
- [ ] 3.5 建立 expected/completed/stale case 索引，拒绝合并 fingerprint 不一致的结果
