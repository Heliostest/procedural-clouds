...
- [ ] 6.1 运行 `npm run typecheck` 与 `npm run build`
- [ ] 6.2 运行 `npm run test:genus-dispatch`，确认十属 evaluator 和现有 density shader 语义未改
- [ ] 6.3 代码分析确认 Legacy Cached/Hybrid 的 update-rate、wind threshold、ping-pong index、cacheBlend、dispatch 数与 pass 顺序保持一致
- [ ] 6.4 人工 A/B：Legacy + Cached/Hybrid 的 Normal 与 density debug 无明显视觉差异；记录这是人工签核，不声称 GPU timing 等价
- [ ] 6.5 请求 Recipe V2，确认 UI/HUD 显示 requested=V2、active=Legacy、reason=not implemented，画面仍由 Legacy 正常输出
- [ ] 6.6 验证 Realtime 基础兼容：仅确认跳过 cache encode 和输出有限，不做性能验收
- [ ] 6.7 运行 `openspec validate add-density-cache-producer-seam --strict --no-interactive`
- [ ] 6.8 W1 完成前不创建或实施 W2 pipeline isolation change
