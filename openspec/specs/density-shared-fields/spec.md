...
Recipe V2 SHALL 定义独立的只读 shared-field sampling ABI：group 2 binding 0 为 filtering sampler、binding 1 为 Base `texture_3d<f32>`、binding 2 为 Detail `texture_3d<f32>`、binding 3 为 Macro `texture_2d<f32>`。Sampler SHALL 使用 repeat addressing 与 linear min/mag filtering；3D atlas SHALL 使用硬件三线性采样。

共享 sampling helper SHALL 支持 body-local normalized coordinate、有限 scale/rotation、seed-derived periodic offset、累计风平流与至多一次低频 coordinate warp。时间连续性 SHALL 优先通过连续坐标平流获得，MUST NOT 通过每帧重建 atlas 或在主路径计算完整 4D 动画噪声。W5 density cache entry MUST NOT 调用该 helper；首次非零调用必须由 W6 或后续独立 change 批准。
...
#### Scenario: W5 Cache 仍无采样

- **WHEN** W5 Recipe V2 Cached/Hybrid 编码正常 density cache pass
- **THEN** cache entry 的可达调用图 SHALL 不包含 `textureSample*` 或 shared sampling helper，output SHALL 继续为有限 RGBA 零值
