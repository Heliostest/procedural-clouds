...
#### Scenario: 按索引取特效强度
- **WHEN** 着色器以整数索引访问预设数组的 `p7` 特效字段
- **THEN** 返回的 `sunDiscVisible`/`haloEffect`/`internalLightning` SHALL 与 `CLOUD_PRESETS` 中对应预设一致
