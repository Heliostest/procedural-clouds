...
## Purpose
城建风格（天际线 2 类）镜头控制：可平移 look-at 目标、球坐标环绕与缩放，并与 gizmo、仿真时钟共存。
...
系统 SHALL 保留指针拖拽环绕与滚轮缩放，并对角度与距离使用平滑跟随（惯性）。系统 SHALL 允许有限仰视，但 MUST 将相机眼点约束在地面净空之上。
...
#### Scenario: 仰视受地表约束
- **WHEN** 用户向下拖拽以仰视，或在仰视姿态下缩放/升降 target
- **THEN** 系统 SHALL 限制俯仰角，使相机眼点高度不低于场景派生的最小地面净空，MUST NOT 将 eye 置于地面以下
