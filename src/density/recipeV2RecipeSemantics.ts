...
export const DENSITY_V2_RECIPE_BANKS: Readonly<Record<CloudGenus, DensityV2RecipeParameterBank>> = Object.freeze({
  ...
  altostratus: DISABLED_BANK,
  nimbostratus: DISABLED_BANK,
  cirrus: DISABLED_BANK,
  cirrostratus: DISABLED_BANK,
  cirrocumulus: DISABLED_BANK,
});

export const DENSITY_V2_ENABLED_GENERA = Object.freeze(['cumulus', 'stratus'] as const);
...
export function verifyDensityV2RecipeSemantics(): void {
  ...
  if (enabled.join(',') !== [...DENSITY_V2_ENABLED_GENERA].sort().join(',')) {
    throw new Error(`Density V2 W6 enabled set mismatch: ${enabled.join(',')}`);
  }
  ...
  if (STRATUS_BANK.sampleLimits.join(',') !== '2,0,0,0'
    || CUMULUS_BANK.sampleLimits.join(',') !== '3,1,0,0') {
    throw new Error('Density V2 W6 static sample limits changed');
  }
}
