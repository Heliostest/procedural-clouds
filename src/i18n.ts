...
const DICT: Dict = {
  ...
  worldStepEnabled: { en: 'World-scale March', zh: '世界尺度步进' },
  worldStepMaxIterations: { en: 'Max Iterations', zh: '最大迭代次数' },
  worldStepMinMeters: { en: 'Min Step (m)', zh: '最小步长（米）' },
  worldStepMaxMeters: { en: 'Max Step (m)', zh: '最大步长（米）' },
  worldStepMaxRayDistanceMeters: { en: 'Max Ray Distance (m)', zh: '最大射线距离（米）' },
  worldStepPerspectiveScale: { en: 'Perspective Growth', zh: '透视步长增长' },
  worldStepSupportSkipping: { en: 'Support Skip', zh: 'Support 空区跳过' },
  worldStepCandidateSkipping: { en: 'Candidate Skip', zh: '候选网格空区跳过' },
  stochasticSampling: { en: 'STBN Sampling', zh: 'STBN 随机采样' },
  stbnFrozenSlice: { en: 'STBN Slice (-1 auto)', zh: 'STBN 切片（-1 自动）' },
  cloudFrameEnabled: { en: 'Cloud-only Frame', zh: '云层独立帧输出' },
  taaEnabled: { en: 'TAA', zh: '时域抗锯齿 TAA' },
  temporalQuality: { en: 'Temporal Quality', zh: '时域质量' },
  ...
  densityStorageMode: { en: 'Density Storage', zh: '密度存储' },
  qualityMode: { en: 'Quality Mode', zh: '质量模式' },
  detailFreq: { en: 'Wavelength Scale', zh: '波长缩放' },
  detailStrength: { en: 'Global Erosion Multiplier', zh: '全局侵蚀倍率' },
  ...
  debugCloudVelocity: { en: 'Cloud Velocity', zh: '云速度' },
  debugCloudValidity: { en: 'Cloud Validity', zh: '云有效位' },
  debugStbnJitter: { en: 'STBN / IGN Jitter', zh: 'STBN / IGN 抖动' },
  debugWorldSkipReason: { en: 'World Skip Reason', zh: '世界步进跳过原因' },
  debugWorldStepMeters: { en: 'World Step Metres', zh: '世界步长（米）' },
  debugTaauPhase: { en: 'TAAU Phase (TAAU only; non-destructive overlay)', zh: 'TAAU Phase（仅 TAAU；非破坏叠加）' },
  debugTaauRejection: { en: 'TAAU History Rejection (TAAU only; non-destructive overlay)', zh: 'TAAU History Rejection（仅 TAAU；非破坏叠加）' },
  debugW12Erosion: { en: 'W12 Erosion (non-destructive overlay)', zh: 'W12 侵蚀（非破坏叠加）' },
  debugW12FinalMinusRough: { en: 'W12 Final − Rough (signed; non-destructive overlay)', zh: 'W12 Final − Rough（有符号；非破坏叠加）' },
...
const TIPS: Record<string, { en: string; zh: string }> = {
  ...
  worldStepEnabled: { en: 'Use anisotropic meter-scaled primary steps. Off preserves the fixed ray-count path exactly for A/B.', zh: '主步进按水平/垂直米制比例换算；关闭时精确保留固定总步数路径以便 A/B。' },
  worldStepMaxIterations: { en: 'Hard upper bound for primary world-step iterations.', zh: '世界尺度主步进的硬迭代上限。' },
  worldStepMinMeters: { en: 'Minimum primary step measured along the ray in meters.', zh: '沿射线计量的主步进最小米制步长。' },
  worldStepMaxMeters: { en: 'Maximum primary step; Support/candidate skips are separately proven intervals.', zh: '主采样最大步长；Support/候选跳过使用独立的已证明区间。' },
  worldStepMaxRayDistanceMeters: { en: 'Maximum camera-to-sample distance in meters.', zh: '从相机起算的最大采样距离（米）。' },
  worldStepPerspectiveScale: { en: 'Step growth per kilometer, clamped by the min/max bounds.', zh: '每公里的步长增长率，仍受最小/最大步长约束。' },
  worldStepSupportSkipping: { en: 'Skip only outside renderer-owned conservative Body Support snapshots.', zh: '仅在 renderer 持有的保守 Body Support 快照之外跳过。' },
  worldStepCandidateSkipping: { en: 'Hierarchical only: skip complete, generation-matched, non-overflow empty candidate tiles.', zh: '仅分层路径：跳过 complete、代次匹配且非 overflow 的空候选 tile。' },
  stochasticSampling: { en: 'Use the optional 128×128×64 STBN volume for primary-step and local-light sampling; unavailable data falls back to IGN/Halton.', zh: '主步进与局部光照采样使用可选 128×128×64 STBN；资源不可用时回退 IGN/Halton。' },
  stbnFrozenSlice: { en: '-1 animates by frame; 0–63 freezes a deterministic slice for diagnosis.', zh: '-1 随帧切片；0–63 固定切片用于诊断。' },
  cloudFrameEnabled: { en: 'Use cloud-only radiance/transmittance, depth/velocity, and full-resolution composite. Disable for the legacy combined A/B path.', zh: '使用云层独立辐射/透射率、深度/速度与全分辨率合成；关闭后进入旧 combined 路径以便 A/B。' },
  ...
  temporalQuality: {
    en: 'Off: no temporal. Full-res TAA: current truth path (full-resolution cloud current + existing TAA). TAAU 4×4: current pass is 1/4 width and height so raymarched texel count is 1/16 of full-res, then full-res temporal upscale resolve (not “pixels only cut to 1/4”).',
    zh: 'Off：关闭时域。Full-res TAA：全分辨率云 current + 现有 TAA，作为回退真值。TAAU 4×4：current 宽高各为全分辨率 1/4，raymarch texel 数为全分辨率的 1/16，再经全分辨率时域升采样（不是“像素数只降到四分之一”）。',
  },
  taaBlend: { en: 'Full-res TAA history weight only (higher = smoother but more ghosting/lag). Does not affect TAAU 4×4 resolve.', zh: '仅作用于 Full-res TAA 的历史权重（越高越平滑，但拖影/延迟更明显）。不影响 TAAU 4×4 resolve。' },
  ...
  densityStorageMode: { en: 'Recipe V2 only: global coarse cache or W9 hierarchical body-local density bricks.', zh: '仅用于 Recipe V2：选择全局粗缓存或 W9 分层云体局部密度 brick。' },
  ...
  detailFreq: { en: 'Global wavelength scaling for render-time erosion detail.', zh: '渲染期侵蚀细节的全局波长缩放。' },
  detailStrength: { en: 'Global multiplier for render-time erosion amount.', zh: '渲染期侵蚀量的全局倍率。' },
