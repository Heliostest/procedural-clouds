...
const DICT: Dict = {
  ...
  altitudeScale: { en: 'Altitude Scale', zh: '海拔比例' },
  horizontalScale: { en: 'Horizontal Scale', zh: '横向比例' },
...
const TIPS: Record<string, { en: string; zh: string }> = {
  ...
  height: { en: 'Bottom Y of this body in world units (0 = box bottom).', zh: '云体底部世界坐标 Y（0 = 盒底）。' },
  thickness: { en: 'Vertical extent in world units, added on top of Height.', zh: '竖直厚度（世界单位），叠加在“高度”之上。' },
  ...
  showAxes: { en: 'RGB world axes; tick labels use Altitude/Horizontal Scale.', zh: 'RGB 世界坐标轴；刻度标注使用海拔/横向比例。' },
  boxHeight: { en: 'World-space height of the render box (Y: 0 to this value).', zh: '渲染盒体世界高度（Y：0 到此值）。' },
  boxHalfExtent: { en: 'Half-width of the cloud box on X/Z (symmetric).', zh: '云盒 X/Z 半宽（对称）。' },
  altitudeScale: { en: 'Displayed altitude per 1 world Y unit (e.g. 100 = meters). Independent from horizontal scale.', zh: '每 1 世界 Y 单位显示的海拔数值（如 100 表示米）。与横向比例独立。' },
  horizontalScale: { en: 'Displayed length per 1 world X/Z unit. Independent from altitude scale.', zh: '每 1 世界 X/Z 单位显示的横向长度。与海拔比例独立。' },
