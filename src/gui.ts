...
const PRESET_FIELD_RANGE: Record<ShapeKey, [number, number, number]> = {
  ...
  sunDiscVisible: [0, 1, 0.01],
  haloEffect: [0, 1, 0.01],
  internalLightning: [0, 1, 0.01],
...
export function createGui(params: CloudParams, store: BodyStore, timeline: TimelineState, simulation: SimulationState, scenario: ScenarioState, hooks: GuiHooks): CloudGui {
  ...
  function build(): void {
    ...
    tipKey(cacheFolder.add(params, 'densityStorageMode', { 'Global only': 0, 'Hierarchical bricks (W9)': 1 }).name(t('densityStorageMode')), 'densityStorageMode');
