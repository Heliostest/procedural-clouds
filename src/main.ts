...
async function main(): Promise<void> {
  ...
  function updateDebug(sceneClock: number, cpuMs: number, workMs: number): void {
    ...
    lines.push(`W11 temporal: active=${s.activeTemporalMode} requested=${s.requestedTemporalMode}${s.temporalFallbackReason ? ` fallback=${s.temporalFallbackReason}` : ''} current=${s.taauCurrentWidth}×${s.taauCurrentHeight} (texels 1/16) phase=${s.temporalBayerPhase} lowres=${(s.cloudFrameLowResAttachmentBytes / 1048576).toFixed(1)}MiB depthHist=${(s.taauHistoryDepthBytes / 1048576).toFixed(1)}MiB`);
    if (s.activeTemporalMode === 'taau-4x4') {
      lines.push(`W11 TAAU GPU: bg=${s.taauBackgroundMs.toFixed(2)}ms current=${s.taauCurrentMs.toFixed(2)}ms resolve=${s.taauResolveMs.toFixed(2)}ms reject≈${(s.taauHistoryRejectionRatio * 100).toFixed(1)}% [nv=${(s.taauRejectNoVelocityRatio * 100).toFixed(1)}% vp=${(s.taauRejectViewportRatio * 100).toFixed(1)}% d=${(s.taauRejectDepthRatio * 100).toFixed(1)}% o=${(s.taauRejectOpacityRatio * 100).toFixed(1)}%] cloud≈${(s.taauCloudCoveredRejectionRatio * 100).toFixed(1)}% thr=${s.taauCloudOpacityThreshold} phaseN=${s.taauCurrentPhaseSampleCount}/${s.taauNonCurrentPhaseSampleCount} (1/16 sampled)`);
    }
